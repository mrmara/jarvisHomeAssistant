...
class jarvis:
    ...
    def timer(self, speechTxt, client_topic):
        ...
        for i in range(0, len(speechTxt)):
            if (speechTxt[i] == "remaining" and self.ongoing_timer != None):
                remaining_time = self.ongoing_timer.get_remaining_time()
                response = str(remaining_time[0]) + " minutes and " + str(remaining_time[1]) + " seconds"
                self.mqtt.publish(client_topic.split("/request")[0]+"/response", response)
            try:
